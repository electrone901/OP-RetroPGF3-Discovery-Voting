...
const AllLists: React.FC = () => {
  ...
  const filteredProjects =
    selectedCategory === "all" ? lists : lists?.filter(list => list?.likes?.includes(String(address)));
  ...
  if (lists && lists.length === 0)
    return (
      <div className="text-center font-bold text-2xl pt-8">
        <h1 className="text-lightGray text-xl m-4 ">There are no lists yet.</h1>
      </div>
    );
        ...
        <div className="w-full pb-10 mb-5">
          ...
          >
            {filteredProjects?.length === 0 ? (
              <h1 className="text-lightGray text-xl m-4 ">You have not liked any lists yet.</h1>
            ) : (
